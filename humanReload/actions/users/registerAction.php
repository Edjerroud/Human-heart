...
if (isset($_POST['validate'])) {
    ...
    if (empty($_POST['name']) || !preg_match('/^[a-zA-Z0-9_]+$/', $_POST['name'])) {
        ...
        $req = $pdo->prepare('SELECT iduser FROM users WHERE `name`=?');
    ...
    if (empty($_POST['phone']) || !preg_match('/^(?:(?:\+|00)33|0)\s*[1-9](?:[\s.-]*\d{2}){4}$/', $_POST['phone'])) {
        ...
        $req = $pdo->prepare('SELECT iduser FROM users WHERE `phone`=?');
    ...
    if (empty($_POST['address'])) {
        ...
        $req = $pdo->prepare('SELECT iduser FROM users WHERE address=?');
    ...
    if (empty($_POST['zipcode']) || !preg_match('/^\\d{5}$/', $_POST['zipcode'])) {
        ...
        $req = $pdo->prepare('SELECT iduser FROM users WHERE zipcode=?');
    ...
    if (empty($_POST['city'])) {
        ...
        $req = $pdo->prepare('SELECT iduser FROM users WHERE city=?');
